    ...
    internal sealed partial class FormMain : Form
    {
        ...
        private void timer30_Tick(object sender, EventArgs e)
        {
                    ...
                    using (var readStream = new StreamReader(receiveStream, Helpers.Russian.Codepage))
                    {
                        ...
                        if (par.Length >= 3)
                        {
                            if (par[0].Equals("1", StringComparison.Ordinal))
                            {
                                ...
                                double pCurrent, pExpired;
                                var days = 0;

                                if (double.TryParse(par[1], NumberStyles.Any, CultureInfo.InvariantCulture, out pCurrent) && 
                                    double.TryParse(par[2], NumberStyles.Any, CultureInfo.InvariantCulture, out pExpired))
                                {
                                    var tCurrent = UnixTimeStampToDateTime(pCurrent);
                                    var tExpired = UnixTimeStampToDateTime(pExpired);
                                    days = (int)((tExpired - tCurrent).TotalDays);
                                }
                                ...
                                if (AppVars.DoChatTip)
                                {
                                    ...
                                    WriteChatMsgSafe(Tips.GetNextAnswer());
                                    WriteChatMsgSafe($"Дней до истечения лицензии: <FONT color=#008000><b>{days}</b></FONT>. Приятной игры!");
                                }
            ...
            catch
            {
                ...
                AppVars.UserKey = AppVars.UserKeyNeverlandsServerError;
            }
            finally
            {
                response?.Close();
            ...
            OnlineLogOn(0);
            if (!string.IsNullOrEmpty(AppVars.UserKey) && !AppVars.UserKey.StartsWith("!"))
            {

                var cd = AppVars.ServerDateTime;
                for (var i = 0; i < 400; i++)
                {
                    var expiredDate = cd.AddDays(i);
                    var sb = new StringBuilder();
                    var str =
                        $"((+-{AppVars.Profile.UserNick.ToUpperInvariant()}*A*{expiredDate.ToString("yyyyMMdd")}-+))";
                    ...
                    for (var k = 0; k < 16; k++)
                    {
                        sb.Append(m[hashbuffer[k] >> 4]);
                        sb.Append(m[hashbuffer[k] & 0xF]);
                        if (((k + 1)%4) == 0 && (k != 15))
                            sb.Append('-');
                    }

                    var sd = sb.ToString();
                    if (sd.Equals(AppVars.UserKey.Trim(), StringComparison.CurrentCultureIgnoreCase))
                    {
                        if (AppVars.DoChatTip)
                        {
                            AppVars.DoChatTip = false;
                            WriteChatMsgSafe(Tips.GetNextAnswer());
                            WriteChatMsgSafe($"Дней до истечения лицензии: <FONT color=#008000><b>{i}</b></FONT>");
                        }
            ...
            string message;
            switch (AppVars.UserKey)
            {
                case AppVars.UserKeyHostsError:
                    message = "Восстановите файл HOSTS!";
                    break;

                case AppVars.UserKeyAbclientServerError:
                    message = "Ошибка связи с сервером клиента!";
                    break;

                case AppVars.UserKeyNeverlandsServerError:
                    message = "Ошибка связи с сервером игры!";
                    break;

                default:
                    message = "Свяжитесь с автором!";
                    break;
            }


            message += Environment.NewLine +
                "Skype: wmlab.home" + Environment.NewLine +
